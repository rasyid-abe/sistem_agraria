...
*
*/
class Report_penguasaan_tanah extends CI_Controller
{
    ...
    public function penguasaan_tanah_show()
    {
        ...
        $data['arr_breadcrumbs'] = array(
            'Report' => '#',
            'Data ' . $title => 'report_penguasaan_tanah',
        );

        $filter = '';

        $sendiri = $this->penguasaan_tanah_sendiri($filter);
        $orang_lain = $this->penguasaan_tanah_orang_lain($filter);
        $bersama = $this->penguasaan_tanah_bersama($filter);

        $data['sendiri'] = $sendiri;
        $data['orang_lain'] =$orang_lain;
        $data['bersama'] =$bersama;

        $data['total_sendiri'] = $this->report_model->get_total_penguasaan_sendiri($filter);
        $data['total_orang_lain'] = $this->report_model->get_total_penguasaan_orang_lain($filter);
        $data['total_bersama'] = $this->report_model->get_total_penguasaan_bersama($filter);

        $data['provinsi'] = $this->helper_model->get_data_provinsi()->result();

        $this->template->content("report/penguasaan_tanah_view", $data);
    ...
    public function show_filter()
    {
        if(in_array('Filter', $_POST)){
            ...
            $this->load->view('export/penguasaan_tanah', $data);
        }

    }

    public function penguasaan_tanah_sendiri($filter)
    {
        $data = $this->report_model->get_data_sendiri($filter);
    ...
    public function penguasaan_tanah_orang_lain($filter)
    {
        $data = $this->report_model->get_data_orang_lain($filter);
    ...
    public function penguasaan_tanah_bersama($filter)
    {
        $data = $this->report_model->get_data_bersama($filter);
